...
log::info!("idx={}", idx);
                if curr_line_tokens[idx].is_word {
                    let cursor_in_start_pos = curr_line_tokens[idx].position == self.cursor.x;
                    if idx == 0 {
                        // if current word-token is the first token in the line,
                        // check the folded line case with loop
                        if let Some(prev_line_tokens) = self.get_line_tokens(y - 1) { // if the prev line exists
                            if prev_line_tokens[prev_line_tokens.len() - 1].is_word {
                                ...
                                if self.cursor.x != curr_line_tokens[idx].position {
                                    // Make sure that cursor is in token start position
                                    self.cursor.x = curr_line_tokens[idx].position;
                                    self.select_to_cursor_pos();
                                    return;
                                }
                                if prev_line_tokens.len() == 1 && !prev_line_tokens[0].is_word {
                                    self.cursor.y -= 1;
                                    self.cursor.x = 0;
                                    self.select_to_cursor_pos();
                                    return;
                    ...
                    if idx == 1 {
                        // "|sC...|"
                        if let Some(prev_line_tokens) = self.get_line_tokens(y - 1) { // if the previous line exists
                            if prev_line_tokens.len() == 1 && !prev_line_tokens[0].is_word {
                                // "|s|"
                                // "|sC...|"
                        ...
                        // "|WsC...|"
                        self.cursor.x = curr_line_tokens[idx - 1].position - 1;
                        return self.vi_mode_backward_to_word_start();
                    } else if idx > 2 {
                        // "|...sWsC...|"
                        self.cursor.x = curr_line_tokens[idx - 2].position;
                    ...
                    if idx == 0 {
                        ...
                        // "|Wc...|"
                        self.cursor.x = curr_line_tokens[idx].position - 1;
                        return self.vi_mode_backward_to_word_start();
                    } else if idx > 1 {
                        // "|...Wc...|"
                        self.cursor.x = curr_line_tokens[idx - 1].position;
    ...
    fn forward_non_whitespace_words(&mut self) {
        ...
        let curr_tokens = Self::collect_merged_tokens(&curr_line[self.cursor.x + 1..], self.cursor.x + 1);
        let next_tokens = Self::collect_merged_tokens(&next_line, 0);
            ...
            if token_len == 0 { // "|...C|" case
                if next_top != -1 {
                    if !next_tokens[0].is_ws {
                        ...
                        // "|...C|"
                        // "|W"
                        return self.forward_to_possible_next_line();
                    } else {
                ...
                if !curr_tokens[0].is_ws { // "|...CW|", "|...CWs|", "|...CWsW|", "|...CWsWs...|" case
                    ...
                    } else if token_len >= 3 { // "|...CWsW|", "|...CWsWs...|" case
    ...
    // mimic the behavior of vi "W" key
    fn forward_to_possible_next_line(&mut self) {
        let old_y = self.cursor.y;
        let old_x = self.cursor.x;
        let (mut next_top, mut next_line) = self.get_next_line();
        if next_top == -1 {
            return;
        }
        ...
        if token_len == 2 {
            if !next_tokens[0].is_ws {
                ...
                let next2_tokens = Self::collect_merged_tokens(&next2_line, 0);
                if let Some(idx2) = Self::find_first_non_whitespace_token(&next2_tokens) {
                    // "|Ws|"   // "|Ws|"
                    // "|W"     // "|sW"
                    self.cursor.y += 1;
                    self.cursor.x = next2_tokens[idx2].position;
                } else {
                    // "|Ws|"
                    // "|s|"
                    self.cursor.y += 1;
